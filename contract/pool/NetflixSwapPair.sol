...
contract NetflixSwapPair is INetflixSwapPair, NetflixSwapBEP20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'NetflixSwapPair: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'NetflixSwapPair: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'NetflixSwapPair: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(
        ...
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'NetflixSwapPair: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = INetflixSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, 'NetflixSwapPair: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'NetflixSwapPair: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(
        ...
        require(amount0Out > 0 || amount1Out > 0, 'NetflixSwapPair: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'NetflixSwapPair: INSUFFICIENT_LIQUIDITY');
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, 'NetflixSwapPair: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'NetflixSwapPair: INSUFFICIENT_INPUT_AMOUNT');
        {
